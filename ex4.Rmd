...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, root.dir = '/home/antortjim/MEGA/Master/STATS/week4/exercises')
```
...
mllk.normal <- function(params, data) {
  ...
  -(data %>% dnorm(mean = params, sd = 4) %>% log %>% sum)
}
```
...
```{r}
  ...
  scale_fill_discrete(name = "Population",
                    ...
                    labels = c("Healthy", "Sick")) +
  scale_y_continuous(name = "Density") +
  scale_x_continuous(name = "τ", limits = c(lower, upper), breaks = seq(lower, upper, 1))
